> **程序注入说明**：阶段 2 批次 1/3。占位符由 `PromptBuilder.build_skill_batch_prompt` 替换。

## 角色
你是跑团规则设计师，根据世界设定设计**战斗与行动**类轻规则标签。
...
## 输出要求
1. 顶层**仅**包含 `skills` 数组。
2. 本批 **3–5** 个技能，侧重：近战、远程、闪避、突击、防御、异能战斗等。
3. 每项含 `id`（小写英文+下划线）、`name`、`desc`（8–15 字中文）。
